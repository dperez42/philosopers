...
{
	sem_unlink("forks");
	if ((g_table->forks = sem_open("forks", O_CREAT, S_IRWXU, g_table->nb_of_philosophers)) ==
																	SEM_FAILED)
		return (0);
	sem_unlink("write");
	if ((g_table->write = sem_open("write", O_CREAT, S_IRWXU, 1)) == SEM_FAILED)
		return (0);
	sem_unlink("lock");
	if ((g_table->lock = sem_open("lock", O_CREAT, S_IRWXU, 1)) == SEM_FAILED)
		return (0);
    return (1);
}
