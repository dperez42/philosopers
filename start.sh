...
echo "\033[94m[+] In Case of a failed test, please check ./errors_log file for more information\033[0m\n"
...
test_one ()
{
    ...
    if [ $output = "died" ];then
        echo "\033[92m[+] Test #1 Succeeded !\033[0m"
    else
        echo "\033[91m[+] Test #1 Failed !\033[0m"
...
test_two ()
{
    ...
    while [ $i -lt 180 ];do
        ...
        pgrep $1 > /dev/null
        if [ "$?" -ne 0 ];then
            echo "\r\033[91m[+] Test #2 Failed\033[0m"
    ...
    if [ $error -eq 0 ];then
        pkill $1
        echo "\r\033[92m[+] Test #2 Succeeded\033[0m"
    fi
}
...
test_three ()
{
    ...
    while [ $i -lt 180 ];do
        ...
        pgrep $1 > /dev/null
        if [ "$?" -ne 0 ];then
            echo "\r\033[91m[+] Test #3 Failed\033[0m"
    ...
    if [ $error -eq 0 ];then
        pkill $1
        echo "\r\033[92m[+] Test #3 Succeeded\033[0m"
    fi
}
...
test_four ()
{
    ...
    if [ "$?" -eq 1 ];then
        ...
        if [ $lines -ge $4 ];then
            echo "\t\033[92m[+] Test #4-$5 Succeeded\033[0m"
        else
            echo "\t\033[91m[+] Test #4-$5 Failed\033[0m"
...
test_five ()
{
    ...
    while [ $i -le 10 ];do
        ...
        if [ "$?" -eq 1 ];then
            ...
            if [ $i -gt 1 ];then
                ...
                x=${x#-}
                if [ $x -gt 10 ];then
                    printf "\r\033[91m[+] Test #5 Failed\033[0m\n"
    ...
    if [ $error -eq 0 ];then
        echo "\r\033[92m[+] Test #5 Succeeded\033[0m"
    fi
}
...
test_six ()
{
    ...
    forks=$(pgrep $1 | wc -l)
    if [ "$forks" -eq 11 ];then
        printf "\r\033[92m[+] Test #6 Succeeded\033[0m\n"
    else
        printf "\r\033[91m[+] Test #6 Failed\033[0m\n"
